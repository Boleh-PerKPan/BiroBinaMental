        <div class="new-panel">
            <div>
                <a href="<?= base_url()?>guest/berita_view"><h4 class="nav-style">Berita Utama</h4></a>
            </div>
            <div class="card mb-3 new-panel">
                <img src="<?=base_url()?>assets/files/images/pendd_char.png" class="card-img-top" alt="...">
                <div class="card-body">
                    <h5 class="card-title">Card title</h5>
                    <p style="text-align:right" class="article-item">
                        <i class="fas fa-tag"></i> Berita Utama &nbsp; &nbsp;
                        <i class="fas fa-user"></i> Dipublish oleh &nbsp; &nbsp;
                        <i class="fas fa-calendar-alt"></i> waktu publish &nbsp; &nbsp;
                    </p>
                    <p class="card-text">This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer...</p>
                    <a href="<?=base_url()?>/guest/detail_berita/berita1" class="btn btn-success" role="button">Selengkapnya &raquo;</a>
                </div>
            </div>
        </div>
        <div class="new-panel" id="beritaterkait">
            <div>
                <a href="<?= base_url()?>guest/berita_view"><h4 class="nav-style">Berita Terkait</h4></a>
            </div>
            <div class="card new-panel" style="">
                <div class="row no-gutters" style="max-height: 25rem;">
                    <div class="col-md-2">
                        <img src="<?= base_url()?>assets/files/images/sumbarprof_logo.png" style="height:5rem; width : 5rem; margin:1rem;" alt="...">
                    </div>
                    <div class="col-md-10">
                        <div class="card-body">
                            <h6 class="card-title">Berita terkait 1</h6>
                            <small>
                                <p class="card-text">
                                    <i>katgori &nbsp; publish oleh &nbsp; waktu publis </i><br>
                                </p>
                            </small>
                            <a href="<?=base_url()?>/guest/detail_berita/berita1" class="btn btn-success" role="button" style="height:28px; padding-top:0px; margin-top:6px;"><small> Selengkapnya &raquo;</small></a>
                        </div>
                    </div>
                </div>
                <hr>
                <div class="row no-gutters" style="max-height: 25rem;">
                    <div class="col-md-2">
                        <img src="<?= base_url()?>assets/files/images/jam_gadang.jpg" style="height:5rem; width : 5rem; margin:1rem;" alt="...">
                    </div>
                    <div class="col-md-10">
                        <div class="card-body">
                            <h6 class="card-title">Berita terkait 2</h6>
                            <small>
                                <p class="card-text">
                                    <i>katgori &nbsp; publish oleh &nbsp; waktu publis </i><br>
                                </p>
                            </small>
                            <a href="<?=base_url()?>/guest/detail_berita/berita1" class="btn btn-success" role="button" style="height:28px; padding-top:0px; margin-top:6px;"><small> Selengkapnya &raquo;</small></a>
                        </div>
                    </div>
                </div>
                <hr>
                <div class="row no-gutters" style="max-height: 25rem;">
                    <div class="col-md-2">
                        <img src="<?= base_url()?>assets/files/images/sumbarprof_logo.png" style="height:5rem; width : 5rem;  margin:1rem;" alt="...">
                    </div>
                    <div class="col-md-10">
                        <div class="card-body">
                            <h6 class="card-title">Berita terkait 2</h6>
                            <small>
                                <p class="card-text">
                                    <i>katgori &nbsp; publish oleh &nbsp; waktu publis </i><br>
                                </p>
                            </small>
                            <a href="<?=base_url()?>/guest/detail_berita/berita1" class="btn btn-success" role="button" style="height:28px; padding-top:0px; margin-top:6px;"><small> Selengkapnya &raquo;</small></a>
                        </div>
                    </div>
                </div>
                <hr>
                <div style="margin: auto; margin-bottom:1rem;">
                    <a href="<?=base_url()?>guest/berita_view" role="button" class="btn my-btn">Berita Lainnya &raquo;</a>
                </div>
            </div>
        </div>
    <!--ini adalah tutup col yang ada di carousel.php -->
    </div>